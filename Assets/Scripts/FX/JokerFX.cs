...
public class JokerFX : MonoBehaviour
{
    ...
    void Start()
    {
        ManagerObject.instance.soundManager.PlaySound(Sounds.ScoreGetSFX, 0.25f, false);
    ...
    void Update()
    {
        ...
        if (t >= 1f)
        {
            AudioSource.PlayClipAtPoint(ManagerObject.instance.resourceManager.gameSoundClips[Sounds.ScoreGetSFX], transform.position);
            Destroy(gameObject);
        }
    }
}
